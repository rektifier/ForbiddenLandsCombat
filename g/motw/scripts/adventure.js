...
$(document).ready(function () {
    ...
    $("#btn-throw-dice").click(function (e) {
        ...
        //get nr of red
        var nrOfDice = $('input[name=dice-toroll]:checked').val();
        var totalResult = '';

        if (isEmpty(nrOfDice) == false && nrOfDice !== "0") {
            var diceService = new DiceRoll(nrOfDice);
            diceService.rolls[0].forEach(function (result) {
                rolledDice.push(result);
            });

            var resultMsg = '';
            if(diceService.total <= 6)
            {
                resultMsg = '0-6 Fail. Mark xp!';
            }

            if(diceService.total >= 7 && diceService.total <= 9)
            {
                resultMsg = '7-9 Mixed success.';
            }

            if(diceService.total >= 10)
            {
                resultMsg = '10+ Success!';
            }


            totalResult = 'Roll: ' + diceService.output + '<br>' +  'Result: ' + resultMsg;
        }
